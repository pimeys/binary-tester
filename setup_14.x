...
if [ "X${PRE_INSTALL_PKGS}" != "X" ]; then
    ...
    exec_cmd "apt-get install --force-yes -y${PRE_INSTALL_PKGS} > /dev/null 2>&1"
fi
...
## You may also need development tools to build native addons:
     sudo apt-get install --force-yes gcc g++ make
## To install the Yarn package manager, run:
     ...
     sudo apt-get update && sudo apt-get install --force-yes yarn
"""
